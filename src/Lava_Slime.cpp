...
Lava_Slime::Lava_Slime(int ix,int iy)
{
    ...
    desenhaFrame.setNomeImg("lava_slime.png");
    ...
    string nome[] = {"SLIME_DANO", "SLIME_MORTE", "SLIME_PULO", "SLIME_QUICA", "SLIME_GOTA"};
    audioEnt.inicializaAudio(nome, 5);
}
Lava_Slime::~Lava_Slime()
{
    for (int i = 0; i < 3; i++)
...
void Lava_Slime::atualizaComportamento()
{
...
void Lava_Slime::atualizaColisoes()
{
...
void Lava_Slime::atualizaEstados()
{
    if(jogadores[0]==NULL&&jogadores[1]==NULL)
    {
        estado=SLI_PARADO;
            ...
            desenhaFrame.getTempoEstado()>=tFrame[estado][desenhaFrame.getFrameAtual() - estFrame[estado][0]])
    {
        ...
        atirar=true;
        al_set_timer_count(timerEnt[0],0);
        al_set_timer_count(timerEnt[1],60);
    }
...
void Lava_Slime::construiFrames()
{
    int i;
...
void Lava_Slime::mudaEstado(int nEstado)
{

}
void Lava_Slime::criaProjetil()
{
    if((al_get_timer_count(timerEnt[1])>tempoProjetil)&&!(estado==SLI_MORRENDO))
    {
        for(int i=0; i<3; i++)
        {
            if(!(gotas[i]->getFuncionando()))
            {
                gotas[i]->setFuncionando(true);
                gotas[i]->getPos().setEspaco(pos.getEspaco());
                ...
                {
                    gotas[i]->setEst(0);
                    if (!audioEnt.audioTocando("SLIME_GOTA"))
                        audioEnt.tocaAudio("SLIME_GOTA", 1, ALLEGRO_PLAYMODE_ONCE, 0);
                }
...
void Lava_Slime::inicializaEntidade()
{
    for (int i = 0; i < 3; i++)
    {
        gotas[i] = new Gota_Lava(Vetor_R2::vetor(-200,-200));
        gotas[i]->setMapaAtual(mapaAtual);
        gotas[i]->setPtrJ(NULL);
        gotas[i]->setListaEnt(listEnt);
        listEnt->incluiEntidade(static_cast<Entidade*> (gotas[i]));
    }
}
void Lava_Slime::setFuncionando(bool f)
{
    funcionando=f;
    al_start_timer(timerEnt[0]);
    al_start_timer(timerEnt[1]);
...
void Lava_Slime::resetaEntidade()
{
...
void Lava_Slime::criaEfeitos()
{

}
void Lava_Slime::paraTimer()
{
    for (int i = 0; i < 3; i++)
    {
