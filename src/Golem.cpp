...
Golem::Golem(int ix,int iy)
{
    ...
    desenhaFrame.setNomeImg("Golem.png");
    ...
    string nome[] = {"GOLEM_NORMAL_DANO", "GOLEM_NORMAL_GOLPE", "GOLEM_NORMAL_MORTE", "GOLEM_PASSO"};
...
void Golem::atualiza()
{
    if(funcionando)
    {
        ...
        if(vida <= 0 && estado!=GOL_MORRENDO)
        {
            ...
            if (!audioEnt.audioTocando("GOLEM_NORMAL_MORTE") && funcionando && desenhaFrame.getEstado()!=GOL_MORRENDO)
            {
                audioEnt.tocaAudio("GOLEM_NORMAL_MORTE", 1, ALLEGRO_PLAYMODE_ONCE, 0);
                mudaScore(score);
            }
...
void Golem::criaProjetil()
{
    if((estado==GOL_ATAQUE) && desenhaFrame.getFrameAtual() == 19 && desenhaFrame.getTempoEstado() == 1)
    {
        if (!audioEnt.audioTocando("GOLEM_NORMAL_GOLPE"))
            audioEnt.tocaAudio("GOLEM_NORMAL_GOLPE", 1, ALLEGRO_PLAYMODE_ONCE, 0);
        for(int i=0; i<3; i++)
        {
        ...
        if(dir)
        {
            atkGolem[0]->getPos().setEspaco(pos.getEspaco()+(Vetor_R2::vetor(-107,0)));
            atkGolem[1]->getPos().setEspaco(pos.getEspaco()+(Vetor_R2::vetor(-100,-90)));
            atkGolem[2]->getPos().setEspaco(pos.getEspaco()+(Vetor_R2::vetor(-35,-126)));
        }
        else
        {
            atkGolem[0]->getPos().setEspaco(pos.getEspaco()+(Vetor_R2::vetor(107,0)));
...
void Golem::atualizaComportamento()
{
    if(!(jogadores[0]==NULL&&jogadores[1]==NULL))
    {
        ...
        if(((pos.getEspaco().getX() < alvo->getPos().getEspaco().getX()&&
                pos.getEspaco().getX()+borda.getX() +40 > alvo->getPos().getEspaco().getX()-alvo->getBorda().getX()) ||
    ...
    if(tomandoDano && !al_get_timer_started(timerEnt[0]))
    {
        ...
        if (vida>0)
            audioEnt.tocaAudio("GOLEM_NORMAL_DANO", 1, ALLEGRO_PLAYMODE_ONCE, 0);
    }
    if(al_get_timer_count(timerEnt[0])>45)
    {
        tomandoDano=false;
...
void Golem::criaEfeitos()
{

}

void Golem::inicializaEntidade()
{
    atkGolem[0] = new Ataque_Generico(Vetor_R2::vetor(-200,-200), Vetor_R2::vetor(40,67),20,4);
    atkGolem[1] = new Ataque_Generico(Vetor_R2::vetor(-200,-200), Vetor_R2::vetor(24,17),20,4);
    atkGolem[2] = new Ataque_Generico(Vetor_R2::vetor(-200,-200), Vetor_R2::vetor(52,6),20,4);
    for(int i=0; i<3; i++)
    {
